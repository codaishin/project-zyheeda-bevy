...
mod tests {
	...
	fn sync_activity() {
		...
		assert_eq!(
			...
			(
				app.world().resource::<GameStatesContext>().activity,
				&app.world().resource::<GameStatesContext>().ui,
			),
		);
	}
	...
	fn act_only_once() {
		...
		app.update();
		app.world_mut().resource_mut::<GameStatesContext>().activity = ActivityState::Save;
		app.world_mut().resource_mut::<GameStatesContext>().ui = HashSet::from([UIState::Hud]);
		app.update();
