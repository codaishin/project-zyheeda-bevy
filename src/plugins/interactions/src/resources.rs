...
pub(crate) struct InteractionsTracker(HashMap<SortedEntities, InteractionsCount>);

impl Track<InteractionEvent> for InteractionsTracker {
	fn update(
		&mut self,
...
mod tests {
	...
	#[test]
	fn single_start() {
		let mut track = InteractionsTracker::default();
		let start = InteractionEvent::of(ColliderRoot(Entity::from_raw(42)))
			.collision(Collision::Started(ColliderRoot(Entity::from_raw(43))));

		assert_eq!(TrackState::Changed, track.update(&start));
	}

	#[test]
	fn multiple_starts() {
		let mut tracker = InteractionsTracker::default();
		let start = InteractionEvent::of(ColliderRoot(Entity::from_raw(42)))
			.collision(Collision::Started(ColliderRoot(Entity::from_raw(43))));

		assert_eq!(
			[TrackState::Changed, TrackState::Unchanged],
			[tracker.update(&start), tracker.update(&start)]
		);
	}
	...
	fn multiple_starts_of_different_interactions() {
		...
		assert_eq!(
			...
			[tracker.update(&start_a), tracker.update(&start_b)]
		);
	}
	...
	fn single_end() {
		...
		assert_eq!([TrackState::Unchanged], [tracker.update(&end)]);
	}
	...
	fn start_end() {
		...
		assert_eq!(
			[TrackState::Changed, TrackState::Changed],
			[tracker.update(&start), tracker.update(&end)]
		);
	}
