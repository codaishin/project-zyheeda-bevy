...
#[derive(Resource)]
pub(crate) struct AnimationData<TAgent, TAnimationGraph = AnimationGraph, TAnimations = Animations>
where
...
impl<TAgent, TAnimationGraph, TAnimations> AnimationData<TAgent, TAnimationGraph, TAnimations>
where
	TAnimationGraph: Asset,
{
	pub(crate) fn new(
		graph: Handle<TAnimationGraph>,
		animations: HashMap<AnimationAsset, (TAnimations, AnimationMask)>,
	) -> Self {
		Self {
			_a: PhantomData,
			graph,
