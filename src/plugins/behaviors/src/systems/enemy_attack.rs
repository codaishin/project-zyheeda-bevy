...
pub(crate) trait AttackSystem {
	fn attack<TGetAttackSpawner>(
		...
		Self: GetterRef<CoolDown> + Component + Sized,
	{
		for (entity, Attack(target), enemy) in &enemies {
			let CoolDown(cold_down) = enemy.get();
...
mod tests {
	...
	#[derive(Component)]
	struct _Enemy(CoolDown);

	impl GetterRef<CoolDown> for _Enemy {
	...
	fn use_spawn_function() {
		...
		let attacker = app
			.world_mut()
			.spawn((Attack(Entity::from_raw(11)), _Enemy(CoolDown::default())))
			.id();
	...
	fn insert_cool_down() {
		...
		let attacker = app
			...
			.spawn((
				...
				_Enemy(CoolDown(Duration::from_secs(42))),
			))
			.id();
