...
pub(crate) trait SelectBehavior {
	fn select_behavior<TPlayer>(
		...
		colliders: Query<&ChildColliderOf>,
	) -> BehaviorResults
	where
...
fn hit_target(target: Entity, hit: Entity, colliders: &Query<&ChildColliderOf>) -> bool {
	if hit == target {
		return true;
	}
	...
	colliders
		.get(hit)
		.map(|ChildColliderOf(root)| root == &target)
		.unwrap_or(false)
}
...
mod tests {
	...
	fn attack_player_when_in_attack_range_with_los_via_collider_root() -> Result<(), RunSystemError>
	{
		...
		let player = app
			...
			.spawn((GlobalTransform::from_xyz(1., 0., 0.), _Player))
			.id();
		let player_collider = app.world_mut().spawn(ChildColliderOf(player)).id();
		let enemy = app
			.world_mut()
			.spawn((
				GlobalTransform::from_xyz(1., 0., 0.5),
