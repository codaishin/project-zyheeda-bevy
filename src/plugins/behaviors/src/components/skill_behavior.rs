...
mod tests {
	...
	mod motion {
		use super::*;
		use common::traits::handles_skill_behaviors::Spawner;
		...
		mod held {
			...
			fn prefab() -> Result<(), RunSystemError> {
				...
				let spawner = Spawner::Slot(SlotKey::BottomHand(Side::Left));
		...
		mod projectile {
			...
			#[test]
			fn prefab_from_contact() -> Result<(), RunSystemError> {
				let (mut app, entity) = setup();
				let caster = PersistentEntity::default();
				...
				assert_eq!(
					(
						...
						Some(
							&Anchor::<Once>::to_target(caster)
								.on_fix_point(SpawnerFixPoint(Spawner::Slot(SlotKey::TopHand(
									Side::Left
								))))
								.with_target_rotation()
						),
			...
			#[test]
			fn prefab_from_save() -> Result<(), RunSystemError> {
				let (mut app, entity) = setup();
				let caster = PersistentEntity::default();
				let motion = Motion::Projectile {
					caster,
