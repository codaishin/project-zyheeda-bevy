	...
	TLoadout: ThreadSafe + HandlesLoadout,
{
	...
	fn state_control(&self, app: &mut App) {
		...
		let loading_essentials = TGameStates::in_game_state([
		...
		let set_activity = set_activity::<TGameStates::TGameStatesMut, TInput::TInput>(hash_map! {
			...
			ActionKey::Save(SaveKey::QuickLoad) => SettableActivity::Load,
			ActionKey::Save(SaveKey::QuickSave) => SettableActivity::Save,
		});
		let toggle_ui = toggle_ui::<TGameStates::TGameStatesMut, TInput::TInput>(hash_map! {
			ActionKey::Menu(MenuState::ComboOverview) => IngameUI::ComboOverview,
			ActionKey::Menu(MenuState::Inventory) => IngameUI::Inventory,
			ActionKey::Menu(MenuState::Settings) => IngameUI::Settings,
		});
		...
		app.add_systems(
			...
			(
				...
				(set_activity, toggle_ui).run_if(changeable_and_not_loading),
			)
				.chain(),
		);
	}
	...
	fn start_menu(&self, app: &mut App) {
		...
		let load = SettableActivity::Load;
	...
	fn ui_overlay(&self, app: &mut App) {
		let play = SettableActivity::Play;

		app.add_ui::<UIOverlay, TLocalization::TLocalizationServer, TGraphics::TCameraMut, TGameStates>(play)
			...
			.add_systems(
				...
				(
					...
					.run_if(TGameStates::in_game_state([play])).after_plugin(TGameStates::SYSTEMS),
			);
	}
