...
mod tests {
	...
	#[derive(SystemParam)]
	struct _Param<'w, 's>(Query<'w, 's, &'static _Slots>);

	impl EntityContext<AvailableSkills> for _Param<'_, '_> {
		type TContext<'ctx> = _Slots;
	...
	fn setup() -> App {
		...
		app.add_systems(
			Update,
			SkillSelectDropdownCommand::<_Layout>::insert_dropdown::<_Agent, _Param, &'static str>,
		);
