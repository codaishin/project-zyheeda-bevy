	...
	TLoadout: for<'c> EntityContext<Skills, TContext<'c>: ReadSkills>,
{
	for agent in &agents {
		for (entity, panel, action_button) in &buttons {
			...
			let color = get_color_override::<TLoadout, TActionKeyButton>(
				panel,
				action_button,
				agent,
				&param,
...
fn get_color_override<TLoadout, TActionKeyButton>(
	QuickbarPanel { key, .. }: &QuickbarPanel,
	action_button: &TActionKeyButton,
	agent: Entity,
	param: &SystemParamItem<TLoadout>,
) -> Option<ColorConfig>
where
	TActionKeyButton: Component + GetProperty<MouseOverrideActive>,
	TLoadout: for<'c> EntityContext<Skills, TContext<'c>: ReadSkills>,
{
	let skills = TLoadout::get_entity_context(param, agent, Skills)?;
	let skill = skills.get_skill(*key)?;
...
mod tests {
	...
	fn setup() -> App {
		...
		app.add_systems(Update, set_color::<_Agent, _ActionKeyButton, _Param>);
