use crate::components::{Collection, Dad, DadPanel};
use bevy::{
	ecs::{
		component::Component,
		query::With,
...
pub fn drop<
	...
	TKeyDadPanel: Send + Sync + Copy + 'static,
	TResult: From<(Dad<TKeyDad>, DadPanel<TKeyDadPanel>)> + Send + Sync + 'static,
>(
	mut commands: Commands,
	agents: Query<(Entity, &Dad<TKeyDad>), With<TAgent>>,
	panels: Query<(&Interaction, &DadPanel<TKeyDadPanel>)>,
	...
	let Some((.., dad_panel)) = panels.iter().find(is_hovered) else {
		return;
	};

	let mut agent = commands.entity(agent);
	agent.insert(Collection::new([TResult::from((*dad, *dad_panel))]));
	agent.remove::<Dad<TKeyDad>>();
}

fn is_hovered<TDadPanel>((interaction, _): &(&Interaction, &DadPanel<TDadPanel>)) -> bool {
...
mod tests {
	...
	impl<T1, T2> From<(Dad<T1>, DadPanel<T2>)> for _Result<T1, T2> {
		fn from(value: (Dad<T1>, DadPanel<T2>)) -> Self {
			let (dad, dad_panel) = value;
			Self(dad.0, dad_panel.0)
		}
	}
	...
	fn add_result_component() {
		...
		app.world.spawn((Interaction::Hovered, DadPanel(11_f32)));
		app.update();
	...
	fn add_result_component_when_multiple_panels_exist() {
		...
		press_and_release_mouse_left(&mut app);
		app.world.spawn((Interaction::Hovered, DadPanel(11_f32)));
		app.world.spawn((Interaction::None, DadPanel(0_f32)));
		app.update();
	...
	fn no_result_when_interaction_not_hover() {
		...
		press_and_release_mouse_left(&mut app);
		app.world.spawn((Interaction::Pressed, DadPanel(11_f32)));
		app.update();
