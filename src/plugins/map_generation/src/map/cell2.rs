...
impl SourcePath for MapCell2 {
	fn source_path() -> Path {
		Path::from("maps/map.txt")
	}
}

impl GridCellDistanceDefinition for MapCell2 {
	const CELL_DISTANCE: f32 = MapCell2::MODEL_ASSET_CELL_WIDTH;
}
...
impl From<MapWindow> for MapCell2 {
	fn from(value: MapWindow) -> Self {
		if value.focus == 'c' {
			return MapCell2::Floor;
		}
		...
		MapCell2::Wall { walls, details }
	}
}

impl From<&MapCell2> for ModelData {
	fn from(cell: &MapCell2) -> Self {
		match cell {
			MapCell2::Floor => model_data!([("floor", Dir3::NEG_Z)]),
			MapCell2::Wall { walls, details } => {
...
mod tests {
	...
	fn new_empty_cell() {
		...
		let cell = MapCell2::from(window);

		assert_eq!(MapCell2::Floor, cell);
	}
