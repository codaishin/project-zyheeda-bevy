...
impl TryFromTriangles for MeshGridGraph {
		...
		TTriangles: Iterator<Item = [Vec3NotNan; 3]>,
	{
		...
		let mut nodes_facing_same_edge = HashMap::<FacedEdge, Vec<Vec3NotNan>>::default();
...
struct FacedEdge(Vec3NotNan, Vec3NotNan);

impl FacedEdge {
	fn uniform(a: Vec3NotNan, b: Vec3NotNan) -> Self {
		match a.cmp(&b) {
