...
impl NavMesh {
	...
	) -> Option<impl Iterator<Item = [Vec3NotNan; 3]>> {
...
pub(crate) trait TryFromTriangles: Sized + Default {
	...
	where
		TIterator: Iterator<Item = [Vec3NotNan; 3]>;
}
...
mod tests {
	...
	use common::{tools::vec_not_nan::Vec3NotNan, vec3_not_nan};
	...
	struct _Graph {
		triangles: Vec<[Vec3NotNan; 3]>,
	}

	impl _Graph {
		fn sorted_triangles(&self) -> Vec<[Vec3NotNan; 3]> {
