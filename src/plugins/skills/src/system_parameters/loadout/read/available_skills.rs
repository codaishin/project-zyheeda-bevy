...
use common::{
	...
	traits::{
		accessors::get::{EntityContext, GetProperty},
		handles_loadout::{
			AvailableSkills,
			GetSkillId,
			ReadAvailableSkills,
...
impl EntityContext<AvailableSkills> for LoadoutReader<'_, '_> {
		...
		fn matching_with_item() -> Result<(), RunSystemError> {
			...
			app.world_mut()
				.run_system_once(move |loadout: LoadoutReader| {
					let ctx = LoadoutReader::get_entity_context(&loadout, entity, AvailableSkills)
						.unwrap();
