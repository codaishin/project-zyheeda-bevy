...
mod tests {
	...
	fn swap_inventory_slot_keys() {
		let swap = Swap(InventoryKey(42), SlotKey::Hand(Side::Right));

		assert_eq!((InventoryKey(42), SlotKey::Hand(Side::Right)), swap.keys());
	}

	#[test]
	fn swap_slot_inventory_keys() {
		let swap = Swap(SlotKey::Hand(Side::Right), InventoryKey(42));
	...
	fn set_swapped_out_item_in_inventory() {
		...
		let mut swaps = Collection::new([_Swap(_InnerKey(0), SlotKey::Hand(Side::Left))]);
	...
	fn retain_swap_try_again_errors() {
		...
		assert_eq!(
			Collection::new([_Swap(_InnerKey(1), SlotKey::Hand(Side::Right))]),
			swaps
		);
	}
