...
impl From<SpawnSkill> for SpawnSkillDto {
	fn from(value: SpawnSkill) -> Self {
		match value {
			SpawnSkill::GroundTargetedAoe(v) => SpawnSkillDto::GroundTargetedAoe(v.into()),
			SpawnSkill::Projectile(v) => SpawnSkillDto::Projectile(v),
			SpawnSkill::Shield(v) => SpawnSkillDto::Shield(v),
			#[cfg(test)]
