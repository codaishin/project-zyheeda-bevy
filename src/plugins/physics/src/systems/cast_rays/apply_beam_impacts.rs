...
impl CastRays {
	pub(crate) fn apply_beam_impacts(cast_rays: Query<&Self>, mut impacted: Query<&mut Impacted>) {
		for CastRays { results } in cast_rays {
			...
			impacted
				.impact_points
				.insert(point, new_f32!(F32FinitePositive(1.0)));
		}
	}
