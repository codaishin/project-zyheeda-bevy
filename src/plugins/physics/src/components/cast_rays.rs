...
pub(crate) struct CastRays {
	pub(crate) results: HashMap<CastRayFor, RayCastResult>,
}

#[cfg(test)]
impl ApproxEqual<f32> for CastRays {
	fn approx_equal(&self, other: &Self, tolerance: &f32) -> bool {
		if other.results.keys().any(|k| !self.results.contains_key(k)) {
			return false;
		}

		for (cast_for, s_result) in self.results.iter() {
			let Some(o_result) = other.results.get(cast_for) else {
				return false;
			};
			...
			if !s_result.args.approx_equal(&o_result.args, tolerance) {
				return false;
			}
		}
