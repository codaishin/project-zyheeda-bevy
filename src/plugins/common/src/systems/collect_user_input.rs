use crate::{
	components::ui_input_primer::{IsKey, IsPrimed},
...
impl UserInput {
	...
	pub(crate) fn collect<TInput, TPrimed>(
		...
		TPrimed: IsKey + IsPrimed + Component,
	{
		let is_not_primed = |user_input: &UserInput| {
			!primers
				.iter()
				.any(|primer| primer.is_primed() && primer.is_key(user_input))
		};
...
mod tests {
	...
	mod collect {
		use super::*;
		use crate::components::ui_input_primer::IsKey;
		...
		impl IsPrimed for _Primer {
			fn is_primed(&self) -> bool {
				self.primed
			}
		}

		impl IsKey for _Primer {
